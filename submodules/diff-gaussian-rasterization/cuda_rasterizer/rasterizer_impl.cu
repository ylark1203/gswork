...
void CudaRasterizer::Rasterizer::forward1(
	const cudaStream_t& stream,
	int* num_rendered,
	std::function<char* (size_t)> geometryBuffer,
	const int P, int D, int M,
	...
	if (radii == nullptr) radii = geomState.internal_radii;
	...
	// Retrieve total number of Gaussian instances to launch and resize aux buffers
	CHECK_CUDA(cudaMemcpyAsync(num_rendered, geomState.point_offsets + P - 1, sizeof(int), cudaMemcpyDeviceToDevice, stream), debug);
	// CHECK_CUDA(cudaMemcpyAsync(num_rendered, geomState.point_offsets + P - 1, sizeof(int), cudaMemcpyDeviceToHost, stream), debug);
	return;
}

void CudaRasterizer::Rasterizer::forward2(
	const cudaStream_t& stream,
	char* geom_buffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, 
	const int* num_rendered_ptr,
	const float* background,
	const int width, int height,
	const float* colors_precomp,
	float* out_color,
	...
	int num_rendered = *num_rendered_ptr;
